...
// TODO: better name
func wrapper(
	fn func(context.Context, *client.Session, *dagger.Client, *dagger.Environment, *cobra.Command, []string) error,
	...
	return func(cmd *cobra.Command, args []string) error {
		...
		return withEngineAndTUI(cmd.Context(), client.SessionParams{}, func(ctx context.Context, sess *client.Session) (err error) {
			...
			return fn(ctx, sess, c, loadedEnv, cmd, dynamicCmdArgs)
		})
	}
}

func ListChecks(ctx context.Context, sess *client.Session, c *dagger.Client, loadedEnv *dagger.Environment, cmd *cobra.Command, dynamicCmdArgs []string) (err error) {
	rec := progrock.RecorderFromContext(ctx)
...
func RunCheck(ctx context.Context, sess *client.Session, c *dagger.Client, loadedEnv *dagger.Environment, cmd *cobra.Command, dynamicCmdArgs []string) (err error) {
	envChecks, err := loadedEnv.Checks(ctx)
	if err != nil {
		return fmt.Errorf("failed to get environment commands: %w", err)
	}
	...
	for _, envCheck := range envChecks {
		envCheck := envCheck
		subChecks, err := addCheck(ctx, &envCheck, c, sess)
		if err != nil {
	...
	if subCmd.Name() == cmd.Name() {
		...
		for _, leafCmd := range allLeafCmds {
			...
			i++
			// TODO:
			fmt.Fprintf(os.Stderr, "%d!!!!!!!!!!!!!!!!!!!!!!!! Running check: %s\n", i, leafCmd.Name())
...
func addCheck(ctx context.Context, envCheck *dagger.EnvironmentCheck, c *dagger.Client, r *client.Session) ([]*cobra.Command, error) {
	rec := progrock.RecorderFromContext(ctx)
	...
	for _, subCheck := range subChecks {
		subCheck := subCheck
		cmds, err := addCheck(ctx, &subCheck, c, r)
		if err != nil {
