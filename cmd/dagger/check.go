...
func RunCheck(ctx context.Context, sess *client.Session, c *dagger.Client, loadedEnv *dagger.Environment, cmd *cobra.Command, dynamicCmdArgs []string) (err error) {
	...
	for _, envCheck := range envChecks {
		envCheck := envCheck
		subChecks, err := addCheck(ctx, nil, &envCheck, c, sess)
		if err != nil {
...
func addCheck(ctx context.Context, cmdStack []*cobra.Command, envCheck *dagger.EnvironmentCheck, c *dagger.Client, r *client.Session) ([]*cobra.Command, error) {
	rec := progrock.RecorderFromContext(ctx)
