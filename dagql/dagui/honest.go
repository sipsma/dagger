...
// Honest exec-time decomposition.
//
// A row's bar decomposes into the time the op actually spent executing (own
// work) vs the time it was provably blocked on other ops (waiting), using the
...
// HonestSegment is one contiguous stretch of a row's painted time.
type HonestSegment struct {
	Start, End time.Time
...
func (seg HonestSegment) Duration() time.Duration {
	return seg.End.Sub(seg.Start)
}

// HonestBreakdown is the full decomposition for one row.
type HonestBreakdown struct {
	// Segments tile the row's painted intervals in time order. Gaps between
	// segments are dormant time.
	Segments []HonestSegment
	Own      time.Duration
...
// honestMinWait and waiting covers at least honestMinWaitFrac of its painted
// time.
const (
	honestMinWait     = 1 * time.Second
	honestMinWaitFrac = 0.2

	// honestMaxDepth bounds transitive blocker resolution.
	honestMaxDepth = 8
)

type honestWaitWindow struct {
	start, end time.Time
	target     *Span // raw wait target (often an internal resume span)
	reason     string
}

// honestResolvedWin is a wait stretch attributed to the op actually working
// during it.
type honestResolvedWin struct {
	start, end time.Time
...
// HonestBreakdown computes the own-vs-waiting decomposition for the span's
// row at the given time.
func (span *Span) HonestBreakdown(now time.Time) *HonestBreakdown {
	hb := &HonestBreakdown{}
	...
	for _, child := range span.ChildSpans.Order {
		if ival := child.honestInterval(now); ival.End.After(ival.Start) {
			painted = append(painted, ival)
		}
	}
	painted = mergeHonestIntervals(painted)
	if len(painted) == 0 {
		return hb
	}
	...
	var raw []honestWaitWindow
	span.collectHonestWaits(&raw, span, now)
	...
	merged := mergeHonestWaits(raw)

	// Resolve each stretch transitively into per-instant blockers, then
	// smooth out sub-visual slivers.
	var resolved []honestResolvedWin
	for _, w := range merged {
		direct := honestResolveBlocker(w.target)
		via := ""
		if direct != nil {
			via = honestBlockerLabel(direct)
		}
		visited := map[*Span]bool{}
		resolved = append(resolved, resolveHonestChain(w, direct, via, 0, visited, now)...)
	}
	...
	for _, w := range resolved {
		if w.blocker != nil && honestInSubtree(w.blocker, span) {
			continue
		}
		genuine = append(genuine, w)
	}
	resolved = genuine
	resolved = coalesceHonestWins(resolved)
	resolved = absorbHonestSlivers(resolved, paintedTotal)
	...
	for _, p := range painted {
		...
		for _, w := range resolved {
			...
			if start.After(cursor) {
				hb.addSegment(HonestSegment{Start: cursor, End: start})
			}
			seg := HonestSegment{
				Start:    start,
				End:      end,
			...
			if w.blocker != nil {
				...
				seg.Label = honestBlockerLabel(w.blocker)
		...
		if cursor.Before(p.End) {
			hb.addSegment(HonestSegment{Start: cursor, End: p.End})
		}
	}
	...
	total := hb.Own + hb.Waiting
	hb.Material = hb.Waiting >= honestMinWait &&
		total > 0 &&
		float64(hb.Waiting) >= honestMinWaitFrac*float64(total)
	...
	if dominant != nil {
		...
		hb.DominantLabel = honestBlockerLabel(dominant)
	} else {
...
// resolveHonestChain attributes the wait window to the ops actually working
...
func resolveHonestChain(win honestWaitWindow, via *Span, viaLabel string, depth int, visited map[*Span]bool, now time.Time) []honestResolvedWin {
	self := honestResolvedWin{
		start:    win.start,
		end:      win.end,
		blocker:  honestResolveBlocker(win.target),
		indirect: depth > 0,
		via:      viaLabel,
	}
	if win.target == nil || depth >= honestMaxDepth || visited[win.target] {
		return []honestResolvedWin{self}
	}
	...
	var subs []honestWaitWindow
	win.target.collectHonestWaits(&subs, win.target, now)
	for _, child := range win.target.ChildSpans.Order {
		if child.Name == win.target.Name {
			child.collectHonestWaits(&subs, win.target, now)
		}
	}
	...
	if resolved := honestResolveBlocker(win.target); resolved != nil && resolved != win.target && !visited[resolved] {
	...
	mergedSubs := mergeHonestWaits(clipped)
	if len(mergedSubs) == 0 {
		return []honestResolvedWin{self}
	}

	var out []honestResolvedWin
	cursor := win.start
	for _, sub := range mergedSubs {
		...
		out = append(out, resolveHonestChain(sub, via, viaLabel, depth+1, visited, now)...)
		cursor = sub.end
	}
...
// coalesceHonestWins merges adjacent windows with the same blocker.
func coalesceHonestWins(wins []honestResolvedWin) []honestResolvedWin {
	if len(wins) == 0 {
		return nil
	}
...
// absorbHonestSlivers folds windows too small to see (or click) into their
// larger neighbor so bars don't fragment into noise.
func absorbHonestSlivers(wins []honestResolvedWin, paintedTotal time.Duration) []honestResolvedWin {
	if len(wins) < 2 {
		return wins
	}
...
func (hb *HonestBreakdown) addSegment(seg HonestSegment) {
...
// collectHonestWaits appends s's wait edges as raw windows attributed to row.
func (s *Span) collectHonestWaits(dst *[]honestWaitWindow, row *Span, now time.Time) {
	s.collectOwnWaits(dst, row, now)
...
func (s *Span) collectOwnWaits(dst *[]honestWaitWindow, row *Span, now time.Time) {
	for i := range s.Links {
		link := &s.Links[i]
		if !link.IsWait() {
			continue
		}
		target := s.db.Spans.Map[link.SpanContext.SpanID]
		...
		if (link.WaitReason == "call_exec" || link.WaitReason == "singleflight") &&
			target != nil && target.Name == row.Name && honestInSubtree(target, row) {
			continue
		}
		...
		*dst = append(*dst, honestWaitWindow{
...
func (span *Span) collectInferredWaits(dst *[]honestWaitWindow, now time.Time) {
	seen := map[*Span]bool{span: true}
	var visit func(s *Span, depth int)
	visit = func(s *Span, depth int) {
		if depth > honestMaxDepth {
			return
		}
		var candidates []*Span
		add := func(c *Span) {
		...
		for _, c := range candidates {
			resolved := honestResolveBlocker(c)
			if resolved == nil || resolved == c {
				continue
			}
			if resolved == span || honestInSubtree(resolved, span) {
			...
			win := honestWaitWindow{
				start:  c.StartTime,
				end:    c.honestInterval(now).End,
				target: c,
			}
...
func (span *Span) WaitingNow(now time.Time) (*Span, bool) {
	...
	seg, ok := span.HonestBreakdown(now).BlockedNow(now)
	if !ok {
		return nil, false
	}
...
func (hb *HonestBreakdown) BlockedNow(now time.Time) (HonestSegment, bool) {
	if n := len(hb.Segments); n > 0 {
	...
	return HonestSegment{}, false
}

// HonestSupportSpans visits the spans a row's honest decomposition draws on
...
func (span *Span) HonestSupportSpans(visit func(*Span)) {
	if span == nil || span.db == nil {
		return
	}
	seen := map[*Span]bool{span: true}
...
func honestBlockerLabel(blocker *Span) string {
	...
	if argv := blocker.honestExecArgv(0); argv != "" {
		return argv
	}
	...
	if argv := blocker.honestCallArgv(); argv != "" {
		return argv
	}
	return blocker.Name
}

// honestCallArgv formats a withExec-style string-list "args" argument from
// the blocker's call, rendered the same way as the exec argv attribute.
func (span *Span) honestCallArgv() string {
	call := span.Call()
	if call == nil {
		return ""
	}
	for _, arg := range call.Args {
		...
		return honestTruncate(strings.Join(parts, " "))
	}
	return ""
}

func honestTruncate(s string) string {
	if len(s) > 48 {
		return s[:47] + "…"
	}
	return s
...
// honestExecArgv finds the wcprof exec argv stamped on the op's process-run
// span (a grandchild of the call span: call → exec.run → exec.processRun).
func (span *Span) honestExecArgv(depth int) string {
	if raw, ok := span.ExtraAttributes[telemetryattrs.WcprofExecArgvAttr]; ok {
		...
		if err := json.Unmarshal(raw, &enc); err == nil {
			...
			if err := json.Unmarshal([]byte(enc), &argv); err == nil && len(argv) > 0 {
				return honestTruncate(strings.Join(argv, " "))
			}
		}
	}
	if depth >= 2 {
	...
	for _, child := range span.ChildSpans.Order {
		if argv := child.honestExecArgv(depth + 1); argv != "" {
			return argv
		}
	}
	return ""
...
func honestInSubtree(span, root *Span) bool {
...
func (span *Span) honestInterval(now time.Time) Interval {
	end := span.EndTime
...
func mergeHonestIntervals(ivals []Interval) []Interval {
	if len(ivals) == 0 {
		return nil
	}
...
// mergeHonestWaits merges overlapping wait windows into disjoint stretches,
// each keeping the contributor covering most of it.
func mergeHonestWaits(wins []honestWaitWindow) []honestWaitWindow {
	if len(wins) == 0 {
		return nil
	}
	sort.Slice(wins, func(i, j int) bool { return wins[i].start.Before(wins[j].start) })
	var merged []honestWaitWindow
	cur := wins[0]
	contributors := []honestWaitWindow{wins[0]}
	flush := func() {
		best := contributors[0]
	...
	for _, w := range wins[1:] {
		...
		flush()
		cur = w
		contributors = []honestWaitWindow{w}
	}
	flush()
	return merged
}
