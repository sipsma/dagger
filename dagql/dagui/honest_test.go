...
// The honest-breakdown tests model the span shapes the engine actually emits
...
var honestT0 = time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)

func at(sec float64) time.Time {
	return honestT0.Add(time.Duration(sec * float64(time.Second)))
}
...
func TestHonestBreakdownForcingOp(t *testing.T) {
	db, s, w1, w2 := buildLazyChainDB(t)
	span := db.Spans.Map[s]
	hb := span.HonestBreakdown(at(20))

	if !hb.Material {
		t.Fatalf("forcing op should be material: %+v", hb)
	}
	if hb.Own > 100*time.Millisecond {
		t.Fatalf("forcing op own work should be ~0, got %v", hb.Own)
	}
...
func TestHonestBreakdownForcedOpOwnWork(t *testing.T) {
	...
	hb2 := db.Spans.Map[w2].HonestBreakdown(at(20))
	if hb2.Material {
		t.Fatalf("w2 should not be material (hosting its own work): %+v", hb2)
	}
	if want := 10*time.Second + 500*time.Millisecond; hb2.Own != want {
		t.Fatalf("w2 own work should be %v (sync 0.5 + exec 10), got %v", want, hb2.Own)
	}

	// w1's deferred evaluation (r1) spent [3,13] waiting on w2's eval and
	// [13,16] doing its own work.
	hb1 := db.Spans.Map[w1].HonestBreakdown(at(20))
	if !hb1.Material {
	...
	if want := 3*time.Second + 500*time.Millisecond; hb1.Own != want {
		t.Fatalf("w1 own work should be %v (sync 0.5 + tail 3), got %v", want, hb1.Own)
	}
...
func TestHonestBreakdownSubtreeRule(t *testing.T) {
	db := NewDB()
	q := testID(1)
	m := testID(2)
	tw := testID(3)
	...
	hb := db.Spans.Map[m].HonestBreakdown(at(20))
	if hb.Waiting != 0 {
		t.Fatalf("hosting row should have no waiting, got %v (%+v)", hb.Waiting, hb.Segments)
	}
	if hb.Own != 10*time.Second {
		t.Fatalf("hosting row own work should be 10s, got %v", hb.Own)
	}
}

func TestHonestBreakdownCalmWithoutWaits(t *testing.T) {
	db := NewDB()
	db.ImportSnapshots([]SpanSnapshot{
		{ID: testID(1), Name: "plain", StartTime: at(0), EndTime: at(2)},
	})
	hb := db.Spans.Map[testID(1)].HonestBreakdown(at(20))
	if hb.Material || hb.Waiting != 0 || hb.Own != 2*time.Second {
...
func TestWaitingNowLiveBlocked(t *testing.T) {
	...
	// as now advances.
	hbA := span.HonestBreakdown(at(6))
	hbB := span.HonestBreakdown(at(12))
	if hbA.Own > 100*time.Millisecond || hbB.Own > 100*time.Millisecond {
		t.Fatalf("own work should stay ~0 while blocked: %v then %v", hbA.Own, hbB.Own)
	}
...
func TestWaitingNowHostingNotBlocked(t *testing.T) {
	...
	hb := db.Spans.Map[m].HonestBreakdown(now)
	if hb.Waiting != 0 {
...
func TestWaitingNowForcedRowBlockedDeeper(t *testing.T) {
	...
	// Own work must not tick up while blocked: frozen at the sync part.
	hbA := span.HonestBreakdown(at(6))
	hbB := span.HonestBreakdown(at(12))
	if hbA.Own != hbB.Own {
		t.Fatalf("own work must stay frozen while blocked: %v then %v", hbA.Own, hbB.Own)
	}
	if hbA.Own != 500*time.Millisecond {
		t.Fatalf("own work should be the 0.5s sync part, got %v", hbA.Own)
	}
}

func TestHonestBreakdownInferredPastWait(t *testing.T) {
	db := NewDB()
	q := testID(1)
	a := testID(2)
	b := testID(3)
	...
	if want := 600 * time.Millisecond; hb.Own != want {
		t.Fatalf("own work should be sync 0.1 + exec 0.5 = %v, got %v (%+v)", want, hb.Own, hb.Segments)
	}
...
func TestHonestBlockerLabelFromCallArgs(t *testing.T) {
	db := NewDB()
	q := testID(1)
	w := testID(2)
	s := testID(3)
...
func TestLiveChainResolvesToWorkingOp(t *testing.T) {
	...
	seg, _ := a.HonestBreakdown(now).BlockedNow(now)
