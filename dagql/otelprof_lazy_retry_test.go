...
// Chunk 3 convergence-pass regression tests (addressing the Codex review):
...
//     emitting a gate-observable targetless wait (design §3.0.1).
...
// TestLazyEmitNestedOverrideNoCrossStamping is the committed unit test for nested
// lazy override composition (the Chunk 2 implementer's LOW): a deferred-work span
...
func TestLazyEmitNestedOverrideNoCrossStamping(t *testing.T) {
	...
	// And it composes through the loader + gate: each work nests under its own lazy
	// op, no cycles.
	...
	if err := wcotel.CheckStructural(cc, g, wcotel.GateOptions{}).Err(); err != nil {
		t.Fatalf("structural gate must pass on the nested lazy emit: %v", err)
	}
	if opByClass(t, g, "Directory.outerWork").Parent != opByClass(t, g, "resume outerDir") {
		t.Fatal("loader: outer work must nest under the outer lazy op")
	}
	if opByClass(t, g, "Directory.innerWork").Parent != opByClass(t, g, "resume innerDir") {
		t.Fatal("loader: inner work must nest under the inner lazy op")
	}
}
