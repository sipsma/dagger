...
func (c *DagqlCache) Call(
	...
	doNotCache bool, // TODO: should just be getting whole cache config in here, tmp measure
) (_ Result, rerr error) {
	startedAt := time.Now()
	lg := slog.With(
	...
	resultID, resultBlob, hitCache, err := s.checkCache(ctx, metaDigest, parent, argResults, resultDigest)
	if err != nil {
	...
	resultID, err = s.insertCache(ctx, metaDigest, parent, argResults, typedVal, resultDigest)
	if err != nil {
...
func (c *DagqlCache) SelectNth(
	...
	resultID, _, hitCache, err := s.checkCache(ctx, metaDigest, enumRes, []Result{nthInput}, resultDigest)
	if err != nil {
		return nil, fmt.Errorf("failed to check cache: %w", err)
	}
	if hitCache {
	...
	resultID, err = s.insertCache(ctx, metaDigest, enumRes, []Result{nthInput}, nthVal, resultDigest)
	if err != nil {
		return nil, fmt.Errorf("failed to insert cache result: %w", err)
	}
	c.mu.Lock()
...
func (s *Server) checkCache(
	...
	rows, err := s.cache.db.QueryContext(ctx, q, qArgs...)
	if err != nil {
	...
	if _, err := s.cache.db.ExecContext(ctx, insertDigstQ, insertDigestArgs...); err != nil {
...
func (s *Server) insertCache(
	...
	if resID == 0 { // TODO: uggo
		...
		if err := s.cache.db.QueryRowContext(ctx, insertResultQ, insertResultArgs...).Scan(&resID); err != nil {
	...
	if _, err := s.cache.db.ExecContext(ctx, insertCallQ, insertCallArgs...); err != nil {
...
func (s *Server) enumCheckCache(
	...
	nthInput, err := Int(nth).ToResult(ctx, s)
	if err != nil {
		return 0, nil, false, fmt.Errorf("scalar result: %w", err)
	}
	return s.checkCache(ctx, metaDigest, enum, []Result{nthInput}, resultDigest)
}

func (s *Server) enumInsertCache(
	ctx context.Context,
	enum Result,
	nth int,
	val Typed,
	...
	nthInput, err := Int(nth).ToResult(ctx, s)
	if err != nil {
		return 0, fmt.Errorf("scalar result: %w", err)
	}
	return s.insertCache(ctx, metaDigest, enum, []Result{nthInput}, val, resultDigest)
}

func (s *Server) loadResultJSON(
	ctx context.Context,
	resultID int,
) ([]byte, error) {
	var jsonBlob []byte
	...
	case errors.Is(err, sql.ErrNoRows):
		return nil, fmt.Errorf("result %d not found", resultID)

	default:
		return nil, fmt.Errorf("failed to load result %d: %w", resultID, err)
	}
}
...
func (s *Server) ScalarResult(
	...
	case errors.Is(err, sql.ErrNoRows):
		...
		if err := s.cache.db.QueryRowContext(ctx, insertResultQ, insertResultArgs...).Scan(&resultID); err != nil {
