...
func (fe *frontendPlain) renderStep(span *dagui.Span, depth int, done bool) {
	...
	if done {
		...
		hb := span.HonestBreakdown(time.Now())
		var durText string
		if hb.Material {
			// honest: the time the op actually spent executing; note where
			// the rest went since a log line can't be hovered
			durText = dagui.FormatDuration(hb.Own)
			if hb.DominantLabel != "" {
