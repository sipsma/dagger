...
// These tests lock in the honest exec-time rendering: material rows report
// own work as THE duration, a row blocked right now says what it is waiting
// on, and completed rows carry no waiting decoration.

func honestWaitLink(target dagui.SpanID, from, to time.Time) dagui.SpanLink {
...
func honestCauseLink(target dagui.SpanID) dagui.SpanLink {
	return dagui.SpanLink{
		SpanContext: dagui.SpanContext{TraceID: prettyTestTraceID(), SpanID: target},
		Purpose:     "cause",
	}
...
// TestRenderHonestDurationCompleted: the forcing op's row reports ~0 own work
...
func TestRenderHonestDurationCompleted(t *testing.T) {
	t.Setenv("NO_COLOR", "1")
	db := dagui.NewDB()
	...
	db.ImportSnapshots([]dagui.SpanSnapshot{
		...
		{ID: twin, TraceID: prettyTestTraceID(), ParentID: stdout, Name: "the-forcing-op",
			StartTime: at(3), EndTime: at(16), Final: true,
			Links: []dagui.SpanLink{honestWaitLink(resume, at(3), at(16))}},
		{ID: resume, TraceID: prettyTestTraceID(), ParentID: twin, Name: "resume withExec",
			StartTime: at(3), EndTime: at(16), Final: true,
			Links: []dagui.SpanLink{honestCauseLink(forced)}},
	})
