...
// OTel emission for lazy / deferred evaluation (design §3.2 + §3.0.2) — the
...
// edge (a cycle / fallback-anchor; design §2.5).
//
...
// wcprof.parent ?? parentId (engine/wcprof/wcotel), so UI reads parentId and the
...
// without moving any span (design §3.0.2/§3.2).
type lazyParentOverride struct {
	...
	// structure survives. This parent-id test is the §3.0.2 discriminator — a
...
// the *direct* re-pointed lazy-work spans (design §3.0.2). OnStart receives the
...
// wcprof.parent on lazy re-pointed work spans (design §3.0.2). It must be
...
// processor on the same provider (design §9 stamping-processor coverage). The
...
func (wcprofLazyParentProcessor) OnStart(parent context.Context, s sdktrace.ReadWriteSpan) {
	...
	// flatten the work subtree under the lazy op (design §3.0.2 guardrail 2).
...
// beginOTelLazyOp mints the OTel `lazy` op span for a deferred evaluation (design
// §3.2 step 1). It MUST be called under lazyMu, before lazyEvalWaitCh is
// published, so a joiner that observes the in-flight eval always has a valid wait
// target (Invariant T, §3.0.1). It returns the context the callback runs under,
...
//     producer so dagui renders unchanged (§3.2 step 2), and the callback ctx
//     carries the wcprof.parent override so the stamping processor re-homes the
//     direct work spans causally to the lazy op (§3.2 step 3).
