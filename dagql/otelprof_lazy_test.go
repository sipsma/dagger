...
// Emit-path tests for the lazy choke point (design §3.2 / §3.0.2), mirroring the
// Chunk 2 discipline (otelprof_hooks_test.go): drive the REAL emit — the real
// beginOTelLazyOp, the real wcprof.parent stamping processor, the real
// resumedCallbackSpan re-point — against an in-memory SDK tracer, then feed the
// genuinely exported spans through the Chunk 1 loader + structural gate. So the
// fixture↔emit correspondence is machine-checked, not just code-reviewed: the
// processor's parent-id discriminator, the producer/no-producer existence cases,
// and the all-exports coverage (§9) are all exercised on real exported spans.
...
func opByClass(t *testing.T, g *wcanalyze.Graph, class string) *wcanalyze.Op {
	t.Helper()
	...
	t.Fatalf("no op with class %q", class)
	return nil
}

// TestLazyEmitProducerRepointStampsDirectChildrenOnly is the load-bearing
// emit-path test for §3.2's producer-context case + §3.0.2's stamping
// discriminator. It drives the real beginOTelLazyOp (producer context captured ⇒
...
// causal re-home on the DIRECT child only (the descendant is unstamped), and that
// the loader re-homes the work under the lazy op with the §6.1 gate green.
func TestLazyEmitProducerRepointStampsDirectChildrenOnly(t *testing.T) {
	...
	if lazySpan == nil || !lazySpan.SpanContext().IsValid() {
		t.Fatal("lazy op span must be minted and valid (Invariant T wait target)")
	}
	...
	// (3) the loader re-homes the work under the lazy op causally; gate green.
	cc, err := wcotel.Compile(toWcotelSpans(ended))
	if err != nil {
		t.Fatalf("compile real lazy emit: %v", err)
	}
	...
	if err := wcotel.CheckStructural(cc, g, wcotel.GateOptions{}).Err(); err != nil {
		t.Fatalf("structural gate must pass on the real lazy emit: %v", err)
	}
	lazyOp := opByClass(t, g, "resume directory")
	workOp := opByClass(t, g, "Directory.withNewFile")
	descOp := opByClass(t, g, "exec /bin/sh")
	producerOp := opByClass(t, g, "Query.directory")
	if workOp.Parent != lazyOp {
		t.Fatalf("loader must re-home the direct work under the lazy op; got parent %v", workOp.Parent)
	}
	if descOp.Parent != workOp {
		t.Fatalf("the descendant must nest under the work span by parentId; got parent %v", descOp.Parent)
	}
	if producerOp.Parent == nil || len(producerOp.Children) != 0 {
		// the producer keeps its UI children=none for this work: the deferred work
		// is NOT a causal child of the producer (no double-count, design §3.2).
		t.Fatalf("producer must have no causal children (work re-homed to lazy op); got %d", len(producerOp.Children))
	}
}

// TestLazyEmitNoProducerNestsUnderHiddenLazyOp covers §3.2's no-producer-context
...
func TestLazyEmitNoProducerNestsUnderHiddenLazyOp(t *testing.T) {
	...
	if err := wcotel.CheckStructural(cc, g, wcotel.GateOptions{}).Err(); err != nil {
		t.Fatalf("structural gate must pass: %v", err)
	}
	if opByClass(t, g, "exec build").Parent != opByClass(t, g, "Query.withNewFile") {
		t.Fatal("work must be a causal child of the hidden lazy op")
	}
}

// TestWcprofLazyParentProcessorStampsAllExports is the §9 "all client providers
// configured" assertion: the stamp must reach EVERY per-client export. It builds
// a provider mirroring engine/server/session.go — the stamping processor FIRST,
...
func TestWcprofLazyParentProcessorStampsAllExports(t *testing.T) {
	...
	for name, exp := range map[string]*tracetest.InMemoryExporter{"own": own, "parent": parent} {
		if !exportedSpanStampedParent(exp, direct.SpanContext().SpanID(), wantParent) {
			t.Fatalf("%s export: the direct work span lost wcprof.parent=%s — a per-client export path missed the stamp (design §9)", name, wantParent)
		}
		if exportedSpanHasParentAttr(exp, descendant.SpanContext().SpanID()) {
			t.Fatalf("%s export: a descendant must not be stamped (design §3.0.2 discriminator)", name)
		}
	}
}
