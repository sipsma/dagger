# Workspace API cleanup required before merge

PR: https://github.com/dagger/dagger/pull/13217
...
Synthetic workspaces make this split visible because `dag.Workspace(root: dir,
cwd: path)` can honestly support the filesystem view, but cannot honestly
support local-host mutation, config loading, module installation, or workspace
migration without inventing new semantics.

This cleanup is required before merging synthetic workspace support. The goal is
to make the API shape explicit enough that synthetic workspaces do not rely on a
long list of special-case rejections.

## Required decisions

- Define the supported contract for `dag.Workspace(root: dir, cwd: path)`.
  - Must support filesystem APIs: `file`, `directory`, `findUp`.
  - `root` is the workspace boundary and defaults to an empty directory.
  - `cwd` is the current working directory inside `root` and defaults to the
    workspace root.
