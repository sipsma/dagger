...
```
      --agent string              With --trace, focus this restored agent (runtime handle or name) instead of the top-level one
  -l, --list                      List available agents
      --partial                   With --trace, restore what the trace carries enough to restore instead of failing on the first agent it does not
  -r, --resume session[=picker]   Resume a saved session (interactive picker if no id given)
      --trace string              Restore a past session from its Dagger Cloud trace: its agents, their conversations, and its scrollback
```
...
```
      --allow-llm strings    List of URLs of remote modules allowed to access LLM APIs, or 'all' to allow every loaded module for the entire session
