...
func convertOutput(output any, outputType *ast.Type, s *MergedSchemas) (any, error) {
	...
	for objectName, baseResolver := range s.resolvers() {
		...
		// TODO:
		// TODO:
		bklog.G(context.TODO()).Debugf("comparing %s to %s", objectName, outputType.Name())
		...
		if !ok {
			...
			// TODO:
			// TODO:
			bklog.G(context.TODO()).Debugf("resolver for %s is not an idable object resolver: %T", objectName, baseResolver)
			continue
		}
...
func (s *environmentSchema) invoke(ctx *core.Context, cmd *core.EnvironmentCommand, _ any) (map[string]any, error) {
	...
	// TODO:
	// TODO:
	bklog.G(ctx).Debugf("INVOKE CALLED")
	...
	// TODO:
	// TODO:
	bklog.G(ctx).Debugf("INVOKE RESULT: %+v", res)
