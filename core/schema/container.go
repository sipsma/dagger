...
func (s *containerSchema) Resolvers() Resolvers {
	return Resolvers{
		...
		"Container": ObjectResolver{
			...
			"fs":                   ToResolver(s.rootfs), // deprecated
			"withRootfs":           ToResolver(s.withRootfs),
			"withFS":               ToResolver(s.withRootfs), // deprecated
			...
			"exec":                 ToResolver(s.withExec), // deprecated
			"exitCode":             ToResolver(s.exitCode),
			...
			"focus":                ToResolver(s.focus),
			"unfocus":              ToResolver(s.unfocus),
		},
	}
}
...
func (s *containerSchema) focus(ctx *core.Context, parent *core.Container, args any) (*core.Container, error) {
	child := parent.Clone()
	child.Focused = true
	return child, nil
}

func (s *containerSchema) unfocus(ctx *core.Context, parent *core.Container, args any) (*core.Container, error) {
	child := parent.Clone()
	child.Focused = false
	return child, nil
}
