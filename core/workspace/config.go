...
func ApplySelectedEnvOverlay(
	...
	if userCfg != nil {
		if envCfg, ok := userCfg.Env[envName]; ok && workspaceKey != "" {
			if env, ok := envCfg.Workspaces[workspaceKey]; ok {
				if err := applyEnvOverlay(applied, envName, env); err != nil {
					return nil, err
				}
				found = true
...
func EnvNamesForWorkspace(cfg *Config, userCfg *UserConfig, workspaceKey string) []string {
	...
	if userCfg != nil && workspaceKey != "" {
		for name, envCfg := range userCfg.Env {
			if _, ok := envCfg.Workspaces[workspaceKey]; ok {
				envNames[name] = true
			}
		}
	}
