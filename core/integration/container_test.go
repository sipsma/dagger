	...
	t.Run("to absolute dir", func(t *testing.T) {
		for _, useAsTarball := range []bool{true, false} {
			t.Run(fmt.Sprintf("useAsTarball=%t", useAsTarball), func(t *testing.T) {
				...
				if useAsTarball {
					tarFile := ctr.AsTarball(filepath.Base(imagePath))
...
func TestContainerAsTarball(t *testing.T) {
	...
	c, ctx := connect(t)
	ctr := c.Container().From(alpineImage)
	tarFile := ctr.AsTarball("foo.tar")
	output, err := ctr.
		WithMountedFile("/foo.tar", tarFile).
...
func TestContainerMultiPlatformExport(t *testing.T) {
	for _, useAsTarball := range []bool{true, false} {
		t.Run(fmt.Sprintf("useAsTarball=%t", useAsTarball), func(t *testing.T) {
			...
			if useAsTarball {
				tarFile := c.Container().AsTarball(filepath.Base(dest), dagger.ContainerAsTarballOpts{
