...
func TestEnvironmentCmdInit(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		t.Run(tc.testName, func(t *testing.T) {
			...
			output, err := ctr.CallEnvironment().Stderr(ctx)
			require.NoError(t, err)
			cfg := core.EnvironmentConfig{}
			require.NoError(t, json.Unmarshal([]byte(lastNLines(output, 5)), &cfg))
	...
	for _, tc := range []testCase{
		...
		for _, testGitEnvironment := range []bool{false, true} {
			...
			t.Run(testName, func(t *testing.T) {
				...
				require.Contains(t, output, environmentDir)
				require.Contains(t, output, environmentDir+"/dagger.json")
				require.Contains(t, output, environmentDir+"/"+tc.expectedMainFile)
			})
		}
	}
}
