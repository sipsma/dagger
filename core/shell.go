...
func (container *Container) ShellEndpoint(bk *buildkit.Client, progSock string) (string, http.Handler, error) {
	...
	return endpoint, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		/*
				...
				if !ok {
					bklog.G(r.Context()).Debugf("HIJACKING NOT SUPPORTED")
					panic("http.Hijacker not implemented")
				}
				bklog.G(r.Context()).Debugf("ABOUT TO HIJACK THIS THING %T %+v", h, h)
				_, _, err := h.Hijack()
				if err != nil {
					bklog.G(r.Context()).Debugf("HIJACK PLZ: %v", err)
					panic(fmt.Sprintf("Hijack failed: %v", err))
				}
				bklog.G(r.Context()).Debugf("HIJACK SUCCESS")
			if conn, ok := r.Context().Value("dumbhack").(net.Conn); ok {
				w = &overrideHijacker{w, conn}
			}
		*/
		...
		if err != nil {
			// TODO:
			bklog.G(context.TODO()).Debugf("RUHROH: %v", err)
			// FIXME: send error
			panic(err)
		}
		defer ws.Close()
		...
		if err != nil {
			bklog.G(context.TODO()).Debugf("WHYYYYYYY: %v", err)
			panic(err)
		} else {
			bklog.G(context.TODO()).Debugf("OMFG: %s", string(msg))
		}
...
/* TODO:
type overrideHijacker struct {
	http.ResponseWriter
	conn net.Conn
}

func (h *overrideHijacker) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	return h.conn, bufio.NewReadWriter(bufio.NewReaderSize(nil, 0), bufio.NewWriterSize(nil, 0)), nil
}
*/
...
func (container *Container) runShell(
	...
	_, err = WithServices(ctx, bk, container.Services, func() (_ any, rerr error) {
		...
		eg.Go(func() error {
			...
			// TODO:
			bklog.G(ctx).Tracef("proc wait DONZO: %v", waitErr)
