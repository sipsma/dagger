...
func (container *Container) ExitCode(ctx context.Context, bk *buildkit.Client, progSock string) (int, error) {
	content, err := container.MetaFileContents(ctx, bk, progSock, "exitCode")
	if err != nil {
		return 0, err
	}
...
func (container *Container) Publish(
	...
	/*
		...
		res, err := bkClient.Build(ctx, solveOpts, "", func(ctx context.Context, bk *buildkit.Client) (*Result, error) {
			return container.export(ctx, bk, platformVariants)
		}, ch)
		if err != nil {
			return "", err
		}
...
func (container *Container) Export(
	...
	/*
		...
		return host.Export(ctx, exportOpts, bkClient, solveOpts, solveCh, func(ctx context.Context, bk *buildkit.Client) (*Result, error) {
			return container.export(ctx, bk, platformVariants)
		})
	*/
}
