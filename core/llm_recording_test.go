...
func TestEmitHistoryPreservesHeaderArgsAndTerminatesJSON(t *testing.T) {
	sr, ctx := recordingTestRecorder(t)
	recorder := &stateRecorder{}
	provider := sdklog.NewLoggerProvider(sdklog.WithProcessor(recorder))
	ctx = telemetry.WithLoggerProvider(ctx, provider)
	...
	require.Condition(t, func() bool {
		...
		return false
	}, "emitted tool JSON was not newline-terminated")
}

func TestEmitHistoryEmitsAuthoritativePatchResult(t *testing.T) {
	_, ctx := recordingTestRecorder(t)
	recorder := &stateRecorder{}
	provider := sdklog.NewLoggerProvider(sdklog.WithProcessor(recorder))
	ctx = telemetry.WithLoggerProvider(ctx, provider)
	...
	t.Fatal("emitted patch result was not emitted")
}
