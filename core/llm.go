...
type LLMMessageOrigin struct {
	...
	AgentName string               `field:"true" name:"agentName" json:"agent_name,omitempty" doc:"The display name of the sending agent (for AGENT origins) or the observed agent (for EVENT origins)." experimental:"Agent APIs are likely to change."`
	Ref       string               `field:"true" name:"ref" json:"ref,omitempty" doc:"The message's short ref within the receiving agent's runtime, e.g. \"#3\": the deterministic token replies name (send's replyTo) and the message lookup takes." experimental:"Agent APIs are likely to change."`
...
type LLMMessageOriginInput struct {
	...
	AgentName string               `name:"agentName" doc:"The display name of the sending or observed agent." default:"" experimental:"Agent APIs are likely to change."`
...
// propagate any session-resource requirements they carry (e.g. a secret or
// socket reached through the workspace), so those requirements are not lost
...
// new LLM DAG node (via withResponse/withToolResult/withTools selectors).
...
// used both during live step() execution and during history emission. callDigest is the
...
// historyResults carries the authoritative result text so history emission can reproduce
// the same result logs as the live call.
func emitMessageSpan(ctx context.Context, msg *LLMMessage, callDigest string, resultTokens map[string]int64, historyResults map[string]historyToolResult) {
	switch msg.Role {
	case LLMMessageRoleUser, LLMMessageRoleSystem:
		emitUserMessageSpan(ctx, msg, callDigest)
	case LLMMessageRoleAssistant:
		emitAssistantMessageSpan(ctx, msg, callDigest, resultTokens, historyResults)
	}
}
...
func emitAssistantMessageSpan(ctx context.Context, msg *LLMMessage, callDigest string, resultTokens map[string]int64, historyResults map[string]historyToolResult) {
	...
	for _, g := range groups {
		func() {
			...
			if g.kind == LLMContentToolCall {
				block := g.blocks[0]
				if result, ok := historyResults[block.CallID]; ok {
					if result.errored {
...
func (llm *LLM) EmitHistory(ctx context.Context) {
	...
	for _, msg := range llm.Messages {
		...
		emitMessageSpan(ctx, msg, "", resultTokens, historyResults)
	}
}
