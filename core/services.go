...
type startingService struct {
	...
	// otelStartSpanCtx is the OTel service.start span for this start (design
	// §3.4) — the OTel analog of profOpID. Minted under ss.l before this
	// startingService is published to ss.starting (Invariant T, §3.0.1) so every
	// installer that blocks on the start has a valid wait target. Zero when OTel
	// telemetry was not active at start time; a recording joiner then emits a
	// gate-observable targetless wait (the cross-session case, §3.0.1), exactly
...
// beginOTelServiceStart mints the OTel service.start span (design §3.4) — the
...
func (ss *Services) startWithKey(
	...
	for {
		...
		case isStarting:
			...
			profWait := wcprof.BeginWait(ctx, starting.profOpID, wcprof.WaitReasonService)
			// OTel installer wait edge (design §3.4): this installer blocks on the
			// in-flight service start, so credit its blocked interval to the
			// service.start span (the analog of native's BeginWait above).
			// dagql.EmitOTelWait self-gates on a recording waiter and, per Invariant
			// T (§3.0.1), emits a gate-observable targetless wait if the start ran
		...
		default:
			...
			// OTel service.start span (design §3.4): minted here, under ss.l and
			// before the ss.starting publish below (Invariant T, §3.0.1), so every
