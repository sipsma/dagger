...
func (s *CompiledRemoteSchema) resolver(runtimeFS *filesystem.Filesystem) graphql.FieldResolveFn {
	return func(p graphql.ResolveParams) (any, error) {
		...
		sessionContext, err := router.SessionContextOf(p)
		if err != nil {
			return nil, err
		}
		contextBytes, err := sessionContext.MarshalText()
		if err != nil {
			return nil, err
		}
		...
		outputDef, err := outputMnt.Marshal(p.Context, llb.Platform(s.hostPlatform), llb.WithCustomName(name))
		if err != nil {
			return nil, err
		}
