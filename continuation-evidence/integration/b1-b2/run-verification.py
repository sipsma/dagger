...
out = Path('/tmp/b1-b2-integration')
...
checks = [
 ...
 ('unfiltered', priv + ' ./core ./core/schema ./dagql -count=1'),
 ('b1-race-compile', 'go test -race -c ./core -o /tmp/b1-b2-integration/core-race.test'),
 ('b1-race', "sudo -n unshare --mount --propagation private /tmp/b1-b2-integration/core-race.test -test.run '^(TestHTTPStateConcurrentResolveCapture|TestStatelessHTTPProducerIsolation|TestCompletedProducerConcurrentDemand)$' -test.count=1 -test.v"),
 ('b1-race-writer', "sudo -n unshare --mount --propagation private /tmp/b1-b2-integration/core-race.test -test.run '^TestHTTPProducerWriter$' -test.count=1 -test.v"),
 ...
 ('diff-check', 'git diff --check a88e0e0cdd..5483cfc495f93d4777104bda7edede27e33607c9'),
]
results=[]
