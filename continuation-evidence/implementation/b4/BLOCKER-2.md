...
**A — extend explicit eager producer coverage for the required Container schema paths.** Specify and record the existing typed recipes on exclusively owned completed children, retaining their exact parent/source inputs and publication attachment. Keep ordinary eager execution unchanged. Audit the required closure, including the Host/source input availability, then repeat the cold proof. This is a foundation extension, not call-frame replay, and should land as new commits above the parent.
