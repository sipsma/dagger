...
**Status: complete.** The five implementation steps and binding addenda are implemented above the unchanged integrated parent. All 23 focused package selections pass without skips. The cold module proof, both warm orders, mixed downloaded-FS/private-execMeta proof and clean restart controls pass. The separately opted-in default-policy diagnostic also passes and verifies actual removal of the saved report; its earlier unconfigured invocation is recorded only as a skip. Implementation tip: `78eab473803a5f5405bf0d315359bed4b0aa1270`; final evidence is a separate signed commit.
...
The historical parent probe passed again after these selections on the unchanged integrated head. The code matches the council's round-1 pin; this final evidence commit adds only evidence and the requested wording corrections. [Final engine manifest](validation-addendum3-engines.json).
...
| --- | --- |
| Root lease re-key, 1 role | 6.20 ms; 1 attach, 1 stale scan, 1 removal; peak 2 owner leases |
| Root lease re-key, 32 roles | 121.06 ms; 32 attaches, 1 stale scan, 32 removals; peak 64 owner leases |
| Typed collector, 1 output / depth 1 | 3.30 µs, 1,794 B, 17 allocations per scan |
| Typed collector, 128 outputs / depth 1 | 273.25 µs, 133,198 B, 1,185 allocations per scan |
| Typed collector, 512 outputs / depth 3 | 2.00 ms, 882,776 B, 5,446 allocations per scan |
| Concurrent publication, 128 outputs | 139 attempts / 11 retries / 128 accepted; 47.11 ms |
| Concurrent publication, 512 outputs | 137 attempts / 9 retries / 128 accepted; 245.76 ms |
| Native callback baseline | 0.65 ns, zero bytes/allocations per entry |
| Activated native host entry | 1.61 µs, 504 B, 13 allocations per entry |
| Ready File acquisition | 9.74 ms; 1 pin, 1 sync attempt, 0 private bodies |
| Chain File acquisition | 55.36 ms; 1 pin, 1 sync attempt, 0 private bodies |
| Partial-sync retry | 47.61 ms; 1 pin, 2 sync attempts, no repeated download/body |
| Failed content → private File producer | 54.43 ms; 1 pin, 1 sync, 1 private body |
| Private File producer | 40.15 ms; 1 pin, 1 sync, 1 private body |
| Native restored delegation, depth 1 | 11.38 ms; 1 independent accessor ref, pin and child owner-sync step |
| Native restored delegation, depth 8 | 74.23 ms; 8 independent accessor refs, pins and child owner-sync steps |
| Native restored delegation, depth 32 | 315.51 ms; 32 independent accessor refs, pins and child owner-sync steps |
| Scratch eager fixture without recipe | 426.7 ns, 432 B, 4 allocations |
| Same fixture with completed recipe | 618.4 ns, 472 B, 6 allocations; delta 191.7 ns, 40 B, 2 allocations |
| Scratch encoded overhead | 2-byte recipe; 35 added bytes in the native Directory payload |
| Cold scratch / canonical-only / warm | 34.74 / 11.12 / 8.75 ms; private Scratch calls 1 / 1 / 0; zero provider reads |
| Still-pending scratch restart acquisition | 25.99 ms; one private Scratch call, no provider reads |
| Scratch owner-sync retry | 32.37 ms; one private Scratch call across failure and retry |

[Collector/host samples](logs/addendum3-collector-costs.log), [real route/re-key samples](logs/addendum3-route-costs.log), [depth costs](logs/addendum3-delegation-costs.log), [scratch real-store samples](logs/addendum3-scratch-costs.log), [recording benchmark](logs/addendum3-scratch-recording-costs.log). The eager microbenchmark isolates the output allocation and recorder; it excludes the already existing eager snapshot I/O. The real scratch samples include twelve concurrent demanders and normal installation, with zero transferred snapshot bytes. Adding a recipe adds no eager recording I/O; the real resolver check observes exactly its existing single Scratch operation. Delegation creates one independently owned installed view and durable role per hop, with zero producer bodies/downloads in these Ready-ancestor samples; temporary pins release after owner sync, while accessors and owner leases remain with their rows. The existing receiver edges remain. These measurements include capture/probe and graph work, so they are not a constant-time per-hop guarantee.

These are Linux/amd64 (AMD EPYC 9554P, Go 1.26.6) samples, not throughput bounds. The collector race performs 8,192 publications and requires 128 complete accepted scans; retries depend on scheduling. The native host microbenchmark excludes bodies/storage. The chain-restart sample takes 57.40 ms, with counters reset on reopen, so it is not a whole-attempt pin total. Private execution allocation totals and large-egraph scaling remain unmeasured. The current real-engine mixed execMeta demand measured 174.84 ms; that end-to-end sample includes real private execution and is separate from the File route microbenchmarks.
