...
## Final integration: `cfa148371c`, A's `9abe5072d5`, and a native run that failed two bodies

The Coordinator's check was right: the helper had no lock of its own, so concurrent first uses could drop a snapshot under each other. `cfa148371c` adds one per value (not the state lock), with a concurrent test that fails without it under the race detector. A's `9abe5072d5` merged as `90e34e09a3` (one evidence conflict: A appended to `REPORT.md`, which is now the index; the addition went to `REPORT-AUTHOR-A.md`). Seven packages pass (3068, row 37).

The native run at this tip failed two bodies (row 38) where the run at `0fca47daa7` passed all sixteen (row 28) and A's at `9c376138c1` passed. What changed between: my two backing commits, which touch `CacheVolume`, the two mirrors and nothing in sharing; and A's cleanup bounds, which the two failing paths do not use (`fixtureEngine.restart` is unchanged). What the log shows: `DonorAfterImport`'s two-minute wait for the sharing pass's receipt expired and the pass's `beforeCommit` was journaled after the wait, so the pass was late, not absent; `TestWorkspaceCapture`'s nested engine exited with code 1 within 1.3 s of the restart's start, before serving anything, and its stderr is not in the log (the other five restarts in the run, including `DonorReleased/AfterRestart` with its cache volume, passed). The host was four times oversubscribed for the second half of the run (five-minute load 63.5 on 16 CPUs). I read both as load, but I cannot show it from this log, and neither is a cleanup-only failure, so the "do not rerun" ruling does not cover them: I have not rerun and ask for one rerun of the full set, or of the two tests, at the Coordinator's choice. If the Workspace restart fails again with exit 1, the nested engine's stderr at start is the thing to capture.
