...
| `f0a18735d7a02b01c66395c9fc463857064dedd7` | R1, R2, R3, R6, R9 evidence and cost note | Documentation scope, completed-body guard rationale, new cold/mixed results and race log, complete base reader enumeration and mutex-wait cancellation cost. Earlier passed logs remain unchanged, with the two HTTP/Git statuses labeled beside their ledger links. |
| This commit | Post-R8 race evidence | One combined DagQL race invocation for owner-sync cleanup and both Decision 1 snapshot-owner reader cases; all pass, no source changes. |
