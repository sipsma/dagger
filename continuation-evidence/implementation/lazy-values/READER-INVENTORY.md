...
Base `018a0e695b96d0849118a510d359795812571e1b`: 436 matching lines (214 direct pointer matches, 222 helper matches). Reviewed implementation `f1b3ed3aa49ca250fff993c35345f8d3baa334ce`: the preserved 495 matches (248 direct, 247 helper).
...
Decision 1 and R5 ownership-read audit: Container owner synchronization takes the pointer latch and tries the state latch. If state is busy, it releases the pointer latch, waits on the state latch alone, then restarts and revalidates the operation and acquired view. With both held, it skips completed groups and releases both before waiting on a running group's latch. File/Directory retain output publication exclusion and release it before a pending-body wait; evaluated bodies are not waited on. Encoding reads its operation pointer before taking the state latch. Capture, Commit, boot and import continue using the nonblocking reader. The race checks cover reader/reader, reader/body and reader/encoder contention, root and inline selection, quiescent shutdown, and the whole-Container body/diagnostic graph-lock sequence.

R6 corrects the incomplete base input: 61 DagQL matches were omitted from the earlier 375-entry enumeration. All original source hashes still match. [The additional entries](probes/base-dagql-readers.md) classify pending-state queries, reporting helpers, lazy callback dispatch, interface contracts and generic test fixtures. This is an inventory correction, not a finding of 61 missing runtime changes. The reviewed-tip enumeration remains unchanged and explicitly pinned; R5 adds a whole-operation contention regression, and R6 changes the stale contract comments without changing DagQL dispatch ordering.
