...
3. **A typed receiver takes at most one slot per pass.** This is a real departure from design §6, which says a typed raw Container uses the same ordered preparation, and it is the deviation most worth review.

   *Why it does not work today.* A typed prepared store decides at preparation time what representation it will find at Commit, and it decides it from the live value: `filePartStore.expected` is the receiver's current `OutputRev` (`core/part_store.go:211`), the Directory store is the same (`:269`), and `containerPartStore` captures `previous := ctr.acquiredOutput.Load()` and sets `view.Revision = baseRevision + 1` (`:468`). A second ordered typed slot would have to expect the revision and view its prefix is *going* to publish, and nothing in `PartStorePreparer`'s signature lets the caller say so.

   *Option (a): thread an expected base into the typed store.* The values the successor needs all exist at preparation time: for File and Directory the successor's `expected` is the predecessor's `expected + 1`, and for Container the successor's `previous` is the exact `*containerAcquiredOutput` the predecessor's `Publish` installs, with `Revision = baseRevision + 2`. What it costs: `PartStorePreparer` and `PartBatchStorePreparer` change shape (`dagql/cache_part_store.go:22,27`), their three implementers change (`core/part_store.go:211`, `:269`, `:468`, plus the batch wrapper at `:545`), and their three call sites change (`dagql/cache_part_install.go:307`, `dagql/cache_part_lazy.go:193,199`). It also needs a carrier for the predecessor's prepared view, which is a core-owned type DagQL must pass back opaquely, so `PreparedPartStore` grows a way to expose the base its `Publish` will install. That is a new cross-package mechanism in a batch 4 interface the design says keeps its arity, which is why I did not invent it.

   *Option (b): what is implemented.* One typed slot per receiver per pass. What it costs, measured rather than argued: the installing task's own completion is a trigger, so the successor pass fills the next part. `TestSnapshotSharingTypedReceiverFillsOnePartPerPass` shows a typed receiver missing two parts ending with both installed, one per pass, with one pin and one accessor ref each, and a third pass that finds nothing left. Nothing is lost; the cost is one extra pass per additional part - an extra probe of the cohort, its member holds and one counted operation - and a window between the two installs in which the receiver has one part and not the other. An encoded receiver, which is what an imported row is until something decodes it, takes the full ordered sequence in one pass, and every §9.1 and §9.2 multi-part case is encoded.

   I chose (b) as the smaller change and am reporting it rather than settling it.
