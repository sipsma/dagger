...
with an optional `contains:` modifier (§4.6). The six Open Decisions are **resolved** (§8).
...
- In `core/container_exec.go`, immediately **before** the QEMU prepend (`~:2042`,
  i.e. after `metaSpec.Args` is the fully-resolved command — `metaSpec.Args = args`
  where `args, _ = container.command(opts)` combines entrypoint + args, `:323,333` —
  and before the first shim), set
...
it merely duplicates the command that already determines the result. *(Implementer
verification: confirm `execMD` is not re-serialized/re-digested after the capture point
in the run path — it is not in the traced paths.)*
...
core capture (Dockerfile `RUN` via the frontend; internal execs with `execMD == nil`)
carry no `ProfArgs` → empty `Op.Argv` → they **remain the aggregated `exec.processRun`
blob** — consistent across **both** sources, the pre-feature status quo, and crucially
**not a mislabel** (never a shim path). Extending decomposition to those paths is
additive future work: set `ProfArgs` at their arg-resolution site with the same pattern
(or, for a pure chokepoint approach, capture at `Run` entry and strip the leading QEMU
constant — `/.init` is added later and is auto-excluded by capture timing).
...
- `core/container_exec.go` (~`:2042`, **before** the QEMU prepend at `:2043`, after
  `metaSpec.Args` is resolved at `:333`): `if execMD != nil { execMD.ProfArgs =
  slices.Clone(metaSpec.Args) }`. *(Replaces the Round-1 `Run`-entry capture entirely.
  Cache-key-neutral by timing — §4.1a.)*
...
8. **Coverage boundary (B4 scope):** an exec that does **not** flow through the core
   capture (no `ProfArgs`) carries empty `Op.Argv` and **stays the `exec.processRun`
   blob** — consistent across native and OTel, and never a shim mislabel. (Confirms the
   §4.1a scope is a graceful non-goal, not a wrong answer.)
...
- **Coverage extension (post-Stage-1):** decompose non-`withExec` execs (Dockerfile
  `RUN` via the gateway frontend) by setting `ProfArgs` at their arg-resolution site —
  additive, same pattern (§4.1a). Decide if/when a real workload needs it.
...
- **Scope (not a risk — a stated boundary):** Stage 1 decomposes `Container.withExec`;
  execs that bypass the core capture (Dockerfile `RUN`, `execMD == nil`) stay the
  aggregated blob — consistent across sources and never a shim mislabel (§4.1a, §6 test 8).
