...
// RefWithRecomputedImplicitInputs returns a lazy reference that re-plans a
// recipe-form ID under the current request context when it is first queried.
// It is intended for explicitly persisted recipes whose engine-provided
// implicit inputs must be refreshed, such as a resumed LLM session.
func RefWithRecomputedImplicitInputs[T Loadable[T]](c *Client, id ID) T {
	var zero T
	q := c.query.Select("node").
		Arg("id", id).
		Arg("recomputeImplicitInputs", true).
		InlineFragment(zero.XXX_GraphQLType())
	return zero.WithGraphQLQuery(q)
}
