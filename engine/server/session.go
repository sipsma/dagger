...
func (srv *Server) getOrCreateDaggerSession(
	...
	if opts.DetachableSession && srv.terminalSessionIDExists(sessionID) {
		return nil, false, controlError(http.StatusConflict, engine.SessionErrorSessionTerminated,
			fmt.Errorf("session %s was terminated", sessionID))
	}

	// Lock the unpublished session before publication. A concurrent request can
	// find it only after both the registry entry and listener claim exist.
	sess = &daggerSession{
		sessionID:          sessionID,
...
func (srv *Server) ServeHTTPToIndependentClient(w http.ResponseWriter, r *http.Request, listenerID string) {
	...
	if isSessionControlPath(r.URL.Path) {
		controlError(http.StatusForbidden, engine.SessionErrorInvalidRequest,
			errors.New("session control routes are unavailable to independent clients")).WriteTo(w)
		return
	}
	clientMetadata, err := engine.ClientMetadataFromHTTPHeaders(r.Header)
	if err != nil {
	...
	if clientMetadata.DetachableSession || clientMetadata.AttachSession || clientMetadata.AttachmentID != "" {
		controlError(http.StatusForbidden, engine.SessionErrorInvalidRequest,
			errors.New("detachable, observer, and attachment roles are unavailable to independent clients")).WriteTo(w)
		return
	}
	if !engine.CheckVersionCompatibility(engine.NormalizeVersion(clientMetadata.ClientVersion), engine.MinimumClientVersion) {
		http.Error(w, fmt.Sprintf("incompatible client version %s", engine.NormalizeVersion(clientMetadata.ClientVersion)), http.StatusInternalServerError)
