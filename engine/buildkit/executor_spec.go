...
func (m hostBindMountRef) Mount(dbg ...string) ([]mount.Mount, func() error, error) {
	return []mount.Mount{{
		Type:    "bind",
		Source:  m.srcPath,
...
func (w *Worker) generateBaseSpec(ctx context.Context, state *execState) error {
	...
	slog.Info("GENERATE BASE SPEC",
		"execID", state.id,
		"proc", fmt.Sprintf("%+v", state.procInfo.Meta),
	)
...
func (w *Worker) setupRootfs(ctx context.Context, state *execState) error {
	...
	rootMnts, releaseRootMount, err := rootMountable.Mount(fmt.Sprintf("%s %+v", state.id, state.procInfo))
	if err != nil {
	...
	slog.Info("SETUP ROOTFS",
		"execID", state.id,
		"mounts", fmt.Sprintf("%+v", rootMnts),
	)
	...
	for _, mnt := range state.nonRootMounts {
		slog.Info("SETUP ROOTFS SUBMOUNT",
			"execID", state.id,
...
func (w *Worker) runContainer(ctx context.Context, state *execState) (rerr error) {
	...
	runcCall := func(ctx context.Context, started chan<- int, io runc.IO, pidfile string) error {
		...
		// TODO: omfg, cleanup the netns api so we don't need this tmpspect bs
		var nsPath string
	...
	l := slog.With("execID", state.id)
	l.Info("START CONTAINER")
	defer func() {
		l.Info("CONTAINER STOPPED")
	}()

	return exitError(ctx, state.exitCodePath, w.callWithIO(ctx, l, state.procInfo, startedCallback, killer, runcCall), state.procInfo.Meta.ValidExitCodes)
}
