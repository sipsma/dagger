...
func wrapError(ctx context.Context, baseErr error, sessionID string) error {
	...
	// TODO:
	// TODO:
	bklog.G(ctx).Debugf("wrapError: %s %s", baseErr, sessionID)

	var fileErr *llberror.FileActionError
	if errors.As(baseErr, &fileErr) {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: file err")
	...
	if errors.As(baseErr, &slowCacheErr) {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: slow cache err")
	...
	if !errors.As(baseErr, &execErr) {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: not an exec err")
		return baseErr
	}
	...
	if !errors.As(baseErr, &opErr) {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: not an operror err")
		return baseErr
	}
	op := opErr.Op
	if op == nil || op.Op == nil {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: op nil")
		return baseErr
	}
	execOp, ok := op.Op.(*pb.Op_Exec)
	if !ok {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: op not op_exec")
		return baseErr
	}
	...
	// TODO:
	bklog.G(ctx).Debugf("wrapError: stdout bytes %s", string(stdoutBytes))
	stderrBytes, err := getExecMetaFile(ctx, mntable, "stderr")
	if err != nil {
		return errors.Join(err, baseErr)
	}
	// TODO:
	bklog.G(ctx).Debugf("wrapError: stderr bytes %s", string(stderrBytes))

	exitCodeBytes, err := getExecMetaFile(ctx, mntable, "exitCode")
	if err != nil {
		// TODO:
		bklog.G(ctx).Debugf("wrapError: failed to get exit code: %v", err)
		return errors.Join(err, baseErr)
	}
	exitCode := -1
	if len(exitCodeBytes) > 0 {
		...
		if err != nil {
			// TODO:
			bklog.G(ctx).Debugf("wrapError: failed to parse exit code: %v", err)
			return errors.Join(err, baseErr)
		}
	}

	// TODO:
	bklog.G(ctx).Debugf("wrapError: returnin successfully")
	return &ExecError{
		original: baseErr,
